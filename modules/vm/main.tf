...
resource "azurerm_subnet" "IN_SUBNET" {
  ...
  resource_group_name  = azurerm_resource_group.IN_RG.name    #Se ponen así y no con var porque se crean al mismo 
  virtual_network_name = azurerm_virtual_network.IN_VNET.name #tiempo y causaría error de referencia.
  address_prefixes     = ["10.123.1.0/24"]

}

#Crear asociación entre subnet y security group
...
#Crear Network Interface Card - Permite conectar la MV a internet
...
resource "azurerm_linux_virtual_machine" "IN_VM" {
  ...
  admin_ssh_key {
    ...
    public_key = file("./keys/711incident_serve.pub")
  }
  ...
  provisioner "file" {
    ...
    connection {
      ...
      private_key = file("./keys/711incident_serve")
